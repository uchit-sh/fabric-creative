...
class SVGModal extends Component {
	...
	render() {
		...
		return (
			...
			>
				<Form colon={false}>
					<Form.Item label={'common.type'}>
						{form.getFieldDecorator('loadType', {
							initialValue: loadType,
						})(
							<Radio.Group onChange={this.handleChangeSvgType}>
								<Radio.Button value="file">{'common.file'}</Radio.Button>
								<Radio.Button value="svg">{'common.svg'}</Radio.Button>
							</Radio.Group>,
						)}
					</Form.Item>
					<Form.Item label={loadType === 'svg' ? 'common.svg': 'common.file'}>
